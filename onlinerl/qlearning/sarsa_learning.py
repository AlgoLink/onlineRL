...
class SarsaAgent:
    def __init__(
        ...
        logger.info(
            f"SarsaAgent init actions: {self.actions}",
            alpha=self.alpha,
            gamma=self.gamma,
        )
    ...
    def learn(
        self, state, action, reward, next_state, next_state_action, model_id=None
    ):
        """
        ...
        q_next = self.get_Q_value(Q_dict, next_state, next_state_action)
